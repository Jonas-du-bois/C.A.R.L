...
## 2025-03-01 - Avoid Correlated Subqueries for Read-Heavy Stats
**Learning:** `MessageRepository.getTopContacts` was executing a correlated subquery for every contact returned to calculate `messages_received` and `messages_sent`. This caused execution times of ~1.3 seconds per 100 calls on large datasets, as SQLite had to scan the `messages` table for every row in `contacts`.
**Action:** Since `total_messages_received` and `total_messages_sent` are already pre-calculated and maintained in the `contacts` table (via atomic updates in `findOrCreateContact`), reading those values directly eliminates the need for correlated subqueries. Combined with a generated expression index `idx_contacts_total_msgs ON contacts((total_messages_received + total_messages_sent) DESC)`, the query execution time is reduced to ~5ms for 100 calls, a 250x performance improvement. Always prefer reading pre-calculated aggregate columns in read-heavy paths when strict consistency is already maintained on write.
