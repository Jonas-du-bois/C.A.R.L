...
## 2025-03-01 - Avoid Subqueries for Cached Aggregates
**Learning:** `MessageRepository.getTopContacts` used correlated subqueries `(SELECT COUNT(*) FROM messages m WHERE m.contact_id = c.id ...)` to count incoming and outgoing messages per contact. This resulted in an N+1 query problem, executing a scan for every contact returned. The `contacts` table already maintained `total_messages_received` and `total_messages_sent` counters.
**Action:** Updated `getTopContacts` to select the pre-calculated `total_messages_received` and `total_messages_sent` columns directly from the `contacts` table, removing the need for subqueries and significantly improving query performance. Always check if aggregate data is already cached on the parent table before performing expensive `COUNT(*)` subqueries.
