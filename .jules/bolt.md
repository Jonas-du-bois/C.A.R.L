...
## 2026-03-02 - Remove redundant DB query in daily report
**Learning:** `CronService.generateAndSendReport` was calling both `getConversationsForReport` and `getMessagesForReport`, but `getMessagesForReport` was only used as a fallback argument to `#formatBasicReport`, which didn't even use the argument. This caused SQLite to perform a completely redundant table scan and memory allocation for the entire day's messages.
**Action:** Removed `getMessagesForReport` entirely. By relying solely on `getConversationsForReport` and calculating `messagesCount` from the conversations array in JavaScript, the daily report generation database execution time was reduced by ~47%. Dead code that performs database queries should be aggressively pruned.
