...
## 2026-05-25 - Use pre-calculated columns instead of correlated subqueries
**Learning:** The `getTopContacts` query in `MessageRepository` used correlated subqueries to count `messages_received` and `messages_sent` for each contact. This causes a massive performance hit (N+1 subqueries per contact). The `contacts` table already maintained these aggregate values in `total_messages_received` and `total_messages_sent` columns.
**Action:** Modify queries to select the existing pre-calculated aggregate columns directly (`c.total_messages_received as messages_received`) instead of executing `COUNT(*)` subqueries. This optimization provides a significant speedup by avoiding N+1 correlated queries.
