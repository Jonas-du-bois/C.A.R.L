...
## 2026-03-03 - Two-Step Query Optimization for Grouped Message Reports
**Learning:** `MessageRepository.getConversationsForReport` originally fetched *all* messages for *all* contacts across the day and processed them in memory, which was particularly inefficient for brief reports (e.g., top 5 contacts only) leading to unnecessary DB load and slow query execution.
**Action:** Implemented a two-step query optimization using a `limitContacts` parameter. First, it aggregates the top `N` active `contact_id`s in a subquery, then dynamically builds an `IN` clause to fetch the detailed message logs strictly for those specific active contacts. This reduces the number of joined rows and avoids loading unneeded history into Node.js application memory.
