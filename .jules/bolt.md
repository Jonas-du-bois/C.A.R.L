...
## 2026-03-03 - Avoid Redundant Data Fetching for Reports
**Learning:** `CronService` was fetching all raw messages for the day (`getMessagesForReport`) in addition to fetching the same messages grouped by contact (`getConversationsForReport`), even though the raw messages were barely used (only for a log count). This caused an unnecessary `SELECT ... JOIN` database query over potentially hundreds of rows.
**Action:** Removed `getMessagesForReport` entirely and derived required metrics (like total message count) from existing `stats` or `conversations` objects to minimize database load during report generation.
