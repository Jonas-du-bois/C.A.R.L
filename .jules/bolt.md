...
## 2026-06-29 - Optimizing Top Contacts Retrieval
**Learning:** Using `COUNT(*)` subqueries on the `messages` table to retrieve top contacts in `getTopContacts` is inefficient, especially when `total_messages_received` and `total_messages_sent` are already pre-calculated in the `contacts` table. Adding a composite expression index on `(total_messages_received + total_messages_sent) DESC` further speeds up sorting.
**Action:** Modified `getTopContacts` to use the pre-calculated aggregations directly from the `contacts` table instead of subqueries, and added the expression index to `src/repositories/Database.js`. This reduced execution time from ~988ms to ~4ms for 100 queries. Always prefer using pre-calculated aggregations instead of expensive `COUNT(*)` subqueries when possible.
