...
## 2026-05-12 - Optimizing Duplicate Message Checks
**Learning:** Checking for duplicate messages using `getMessageById` was inefficient because it performed a full row `SELECT` with a `JOIN` on the `contacts` table, even though only existence verification was needed. Since WhatsApp duplicate events are frequent, this added unnecessary overhead to the hot path before expensive async operations like `getChat()`.
**Action:** Implemented a lightweight `messageExists(id)` method using `SELECT 1` without JOINs, and moved the duplicate check *before* the expensive asynchronous IPC call `getChatSafe()` in `Application.js`. Benchmarks showed a ~3x speedup (from 340.153ms to 113.955ms for 10,000 checks) for the database query alone, plus the avoided overhead of IPC. Always use `SELECT 1` for lightweight existence checks and place them as early as possible to reduce database and application overhead.
