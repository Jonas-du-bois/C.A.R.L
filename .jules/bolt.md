...
## 2026-06-05 - Optimizing Top Contacts Query
**Learning:** `MessageRepository.getTopContacts` used correlated subqueries (`SELECT COUNT(*)`) to calculate `messages_received` and `messages_sent` for each contact. This created an N+1 overhead and bypassed the already pre-calculated `total_messages_received` and `total_messages_sent` values in the `contacts` table.
**Action:** Updated `getTopContacts` to directly select `total_messages_received` and `total_messages_sent` from the `contacts` table, eliminating the correlated subqueries. Benchmarks showed an 8.62x speedup (1096.42ms -> 127.22ms for 1000 iterations). Always use pre-calculated aggregate columns when available instead of re-aggregating data in the query.
