...
## 2026-03-05 - Avoid Redundant DB Queries for Daily Report
**Learning:** The `CronService` was fetching all messages sequentially via `getMessagesForReport` and then redundantly executing `getConversationsForReport`, despite the latter containing all the necessary message data formatted for the application logic. This caused an unnecessary load on the database querying the same messages twice.
**Action:** Remove `getMessagesForReport` and extract global message metrics (e.g. `messagesCount`) directly from the `stats` retrieved by `getQuickStats` or calculate it implicitly from the conversational context, effectively cutting the database reads in half during daily report generation.