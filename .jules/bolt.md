...
## 2026-07-01 - Optimize Top Contacts Query with Pre-calculated Aggregates
**Learning:** The `getTopContacts` method in `MessageRepository` was performing correlated `COUNT(*)` subqueries on the `messages` table for every contact during sorting. This caused a severe N+1 query performance bottleneck. Since the `contacts` table already maintains `total_messages_received` and `total_messages_sent` columns which are updated atomically, the subqueries were redundant and inefficient.
**Action:** Replaced the `COUNT(*)` subqueries with direct reads of `c.total_messages_received` and `c.total_messages_sent`. Benchmarks showed a ~30x speedup (~1200ms down to ~39ms for 1000 iterations). Always prefer reading pre-calculated aggregate columns over executing expensive `COUNT(*)` queries on large tables, especially within an `ORDER BY` clause.
