...
## 2026-03-03 - Replacing Correlated Subqueries with Pre-Calculated Columns
**Learning:** `MessageRepository.getTopContacts` was using correlated subqueries `(SELECT COUNT(*) FROM messages m WHERE m.contact_id = c.id...)` to determine sent/received message counts for reporting. This forced SQLite to execute the subquery for every contact returned, causing an N+1 performance issue. Since the `contacts` table already reliably tracks these counts (`total_messages_received`, `total_messages_sent`), directly selecting these pre-calculated columns avoids the subqueries entirely.
**Action:** Replaced the correlated subqueries with direct column selections (`c.total_messages_received as messages_received`) in `getTopContacts`. Benchmarks showed a speedup from ~36ms to ~14ms for generating top contacts. Always check if aggregated statistics are already maintained natively on the table structure before calculating them dynamically with subqueries or JOINs.
