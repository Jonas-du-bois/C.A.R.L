...
## 2026-03-10 - Avoiding Correlated Subqueries with Pre-calculated Columns
**Learning:** `MessageRepository.getTopContacts` was performing correlated subqueries (`SELECT COUNT(*) FROM messages ...`) for every contact to calculate their message stats, causing severe latency on large databases. Since the `contacts` table already maintained `total_messages_received` and `total_messages_sent` columns, reading directly from them avoided the subqueries entirely. Additionally, sorting by a computed expression without an index forces a runtime sort.
**Action:** Replaced the correlated subqueries with direct column reads (`c.total_messages_received`, etc.) and added an expression index `CREATE INDEX IF NOT EXISTS idx_contacts_total_msgs ON contacts((total_messages_received + total_messages_sent) DESC);`. This allows SQLite to perform a fast index scan instead of sorting dynamically. Always leverage pre-calculated stats and expression indexes for frequent, heavy ranking queries.
