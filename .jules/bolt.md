...
## 2026-03-02 - Avoid Correlated Subqueries for Stats Retrieval
**Learning:** `MessageRepository.getTopContacts` was using correlated subqueries (`SELECT COUNT(*) FROM messages ...`) to retrieve message counts for each contact. This resulted in significant performance degradation, taking ~700ms for 100 queries, because the subquery ran for every contact row.
**Action:** Replaced the expensive `COUNT(*)` correlated subqueries with direct selects of the `total_messages_received` and `total_messages_sent` fields already tracked and updated in the `contacts` table. Benchmarks confirmed a massive drop in latency, completing 100 queries in ~7ms. Always favor pre-calculated aggregations over on-the-fly subqueries for frequent reads.
