...
## 2026-03-03 - O(1) Reporting Using Pre-Calculated Columns
**Learning:** Functions like `getGlobalStats` and `getTopContacts` were performing full table `COUNT(*)` scans on the `messages` table and correlated subqueries, leading to significant overhead for reporting routes as the database size grew.
**Action:** Replaced expensive `COUNT(*)` calls with `SUM()` aggregations on the pre-calculated `total_messages_received` and `total_messages_sent` columns in the `contacts` table. When performing `SUM()` aggregations that might evaluate against empty tables in SQLite, always wrap the expression in `CAST(COALESCE(SUM(column), 0) AS INTEGER)` to handle nulls and strictly enforce an integer return type for JavaScript consumers. Additionally, an expression index was created to optimize the sorting of top contacts based on total message activity.
