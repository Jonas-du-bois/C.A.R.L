...
## 2024-04-19 - Use pre-calculated column stats instead of N+1 subqueries
**Learning:** `MessageRepository.getTopContacts` was running slow because it executed a `SELECT COUNT(*)` on the `messages` table for both received and sent messages for each row in the `contacts` table (N+1 problem).
**Action:** The `contacts` table already maintained pre-calculated `total_messages_received` and `total_messages_sent` columns, which were updated atomically upon inserting a message. Queries that need these aggregations should pull directly from those pre-calculated columns rather than executing expensive runtime table scans.
