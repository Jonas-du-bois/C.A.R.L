...
## 2025-05-19 - [Optimize getTopContacts by eliminating N+1 subqueries]
**Learning:** The `getTopContacts` method used N+1 correlated subqueries on the `messages` table even though `total_messages_received` and `total_messages_sent` were already pre-calculated in the `contacts` table.
**Action:** Use pre-calculated fields directly from the `contacts` table to optimize frequent top contacts queries and eliminate N+1 subquery bottlenecks.
