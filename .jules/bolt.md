...
## 2026-06-25 - Optimizing Sorting by Computed Values in SQLite
**Learning:** When sorting records by the sum of multiple columns (e.g., `ORDER BY (total_messages_received + total_messages_sent) DESC`), executing subqueries for every row is an extreme performance bottleneck. While removing the subqueries and using pre-aggregated columns helps, the ultimate speedup requires a composite index on the expression itself. However, SQLite requires expressions in `CREATE INDEX` statements to be enclosed in an extra set of parentheses (e.g., `CREATE INDEX idx ON table((col1 + col2) DESC)`). Failing to do so results in a syntax error.
**Action:** When optimizing `ORDER BY` clauses with mathematical expressions, always create an expression index and ensure the expression is wrapped in parentheses. Always verify SQLite schema syntax against an in-memory database during optimization.
