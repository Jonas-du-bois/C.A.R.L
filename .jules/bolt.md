...
## 2024-06-25 - Avoid Correlated Subqueries on Pre-calculated Aggregates
**Learning:** `MessageRepository.getTopContacts` was running correlated subqueries to calculate `messages_received` and `messages_sent` dynamically (`SELECT COUNT(*)` on each row), despite the values being already tracked by `total_messages_received` and `total_messages_sent` columns in the `contacts` table. This led to unnecessary N+1 queries.
**Action:** When a table maintains pre-calculated aggregate fields through operations (like the atomic UPSERT in `findOrCreateContact`), query those columns directly rather than performing costly re-aggregations.
