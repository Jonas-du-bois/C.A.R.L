...
## 2026-03-02 - Use Pre-Calculated Stats Instead of Correlated Subqueries
**Learning:** `MessageRepository.getTopContacts` was executing a correlated subquery for each returned contact `(SELECT COUNT(*) FROM messages m WHERE m.contact_id = c.id ...)` to count incoming and outgoing messages. This introduced significant N+1 query overhead as the database had to perform a full index scan on the `messages` table twice per contact. The `contacts` table already maintained `total_messages_received` and `total_messages_sent` dynamically (updated atomically via `findOrCreateContact` and `updateContactStats`).
**Action:** Replaced the correlated subqueries with direct selects of the `total_messages_received` and `total_messages_sent` attributes. This avoids the O(N) performance hit and relies on O(1) attribute lookup, significantly speeding up contact leaderboard generation on datasets with many messages.
