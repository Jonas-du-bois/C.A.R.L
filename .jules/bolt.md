...
## 2026-06-07 - Replacing N+1 Subqueries in getTopContacts with Pre-calculated Counters
**Learning:** The `MessageRepository.getTopContacts` method used correlated subqueries (`SELECT COUNT(*) FROM messages m WHERE m.contact_id = c.id`) to fetch message counts for each contact. This causes massive N+1 overhead on large datasets. Because the application already maintains accurate `total_messages_received` and `total_messages_sent` counters on the `contacts` table, direct selection is much faster.
**Action:** Refactored `getTopContacts` to directly select `c.total_messages_received` and `c.total_messages_sent` instead of running subqueries. Benchmark test on 100,000 messages and 1,000 contacts showed a query time reduction from ~10s to ~11ms. Always prefer reading pre-calculated aggregation columns from the parent table over running N+1 queries on child tables.
