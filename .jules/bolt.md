...
## 2026-04-22 - Avoid Correlated Subqueries when Aggregates are Pre-calculated
**Learning:** `MessageRepository.getTopContacts` was performing a table scan and then executing two correlated `COUNT(*)` subqueries on the `messages` table for *every* contact returned. These counts (`total_messages_received`, `total_messages_sent`) are actually already atomically maintained on the `contacts` table during the `findOrCreateContact` and `updateContactStats` flows.
**Action:** Replaced the expensive `COUNT(*)` subqueries with direct `SELECT` of the existing `c.total_messages_received` and `c.total_messages_sent` columns. This reduced query execution time for a 100-contact dataset significantly (~10,000ms -> ~60ms for 1000 iterations in benchmark). Always check if aggregate values are already materialized on the parent entity before resorting to expensive JOINs or correlated subqueries.
