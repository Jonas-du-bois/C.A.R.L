...
## 2026-03-03 - Avoid Correlated Subqueries for Count Aggregations
**Learning:** `MessageRepository.getTopContacts` was performing two correlated subqueries `(SELECT COUNT(*) FROM messages ...)` for each contact to calculate total received and sent messages. This was highly inefficient and bypassed the `contacts` table's pre-calculated columns which were specifically designed to prevent this exact query pattern. Furthermore, the query was sorting by an un-indexed expression `(total_messages_received + total_messages_sent)`.
**Action:** Refactored `getTopContacts` to read directly from the `total_messages_received` and `total_messages_sent` columns in the `contacts` table. Also added a generated expression index `idx_contacts_total_msgs` on `contacts((total_messages_received + total_messages_sent) DESC)` to optimize the `ORDER BY` clause, reducing query execution time significantly and avoiding a full table sort.
