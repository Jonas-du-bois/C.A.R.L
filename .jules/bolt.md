...
## 2026-06-28 - Optimizing getTopContacts by avoiding subqueries
**Learning:** In `MessageRepository.getTopContacts`, the query was performing two subqueries `(SELECT COUNT(*) FROM messages ...)` for each contact. Because the `messages` table grows linearly with bot usage, scanning it for every contact causes the query to significantly slow down (taking ~1906ms for 10,000 messages in benchmark). The `contacts` table already tracks `total_messages_received` and `total_messages_sent`.
**Action:** Replaced the subqueries with the pre-calculated `total_messages_received` and `total_messages_sent` columns from the `contacts` table, and added an expression index on `((total_messages_received + total_messages_sent) DESC)` to `src/repositories/Database.js`. This reduced the benchmark time from ~1906ms to ~8ms. Always rely on pre-calculated aggregate counters in parent tables rather than performing expensive `COUNT()` joins/subqueries on large child tables when sorting or calculating totals.
