...
## 2026-03-03 - Eliminating Correlated Subqueries for Counters
**Learning:** The `getTopContacts` method used correlated subqueries `(SELECT COUNT(*) FROM messages ...)` to count messages per contact, which is an expensive operation (N+1 query overhead).
**Action:** Changed the query to directly select the pre-calculated aggregate columns `c.total_messages_received` and `c.total_messages_sent` from the `contacts` table, reducing query execution time significantly.
