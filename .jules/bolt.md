...
## 2026-03-02 - Avoiding IPC and JOIN Overheads on Duplicate Message Checks
**Learning:** The application was calling `msg.getChat()`—which initiates a slow, asynchronous IPC call to the Puppeteer browser—before checking if an incoming or outgoing message was a duplicate. In addition, the deduplication check used `getMessageById()`, which executes a `JOIN` with the `contacts` table and loads the entire message payload. This unnecessarily consumed CPU, database, and IPC resources for messages that were ultimately discarded.
**Action:** Created an optimized `messageExists()` method in `MessageRepository.js` using a `SELECT 1` query without joins or heavy column loading. Crucially, moved this deduplication check *before* the `msg.getChat()` call in the event handlers. Always perform cheap, synchronous gating checks (like DB lookups) before executing expensive or asynchronous operations (like IPC calls or network requests).
