...
## 2026-06-03 - Optimizing getTopContacts By Avoiding N+1 Subqueries
**Learning:** `MessageRepository.getTopContacts` used correlated subqueries `(SELECT COUNT(*) FROM messages m WHERE m.contact_id = c.id ...)` to calculate messages received and sent for each contact on every call, which creates an N+1 query problem. The `contacts` table already maintained these counts via `total_messages_received` and `total_messages_sent`.
**Action:** Replaced the expensive subqueries with direct column lookups (`c.total_messages_received as messages_received`, `c.total_messages_sent as messages_sent`). Benchmarks showed a dramatic speedup. Always prefer pre-calculated aggregate fields when available.
