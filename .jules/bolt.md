...
## 2026-03-03 - Optimizing Global Stats and Top Contacts Queries
**Learning:** `MessageRepository.getGlobalStats` and `MessageRepository.getTopContacts` were performing full table scans and correlated subqueries on the `messages` table, respectively. Since `messages` is a high-volume, continuously growing table, these operations became increasingly expensive. However, the application already maintains accurate, pre-calculated counters (`total_messages_received`, `total_messages_sent`) in the much smaller `contacts` table via atomic updates during message insertion.
**Action:** Replaced the full table `COUNT(*)` scans in `getGlobalStats` with O(1) `SUM()` aggregations on the `contacts` table columns. Updated `getTopContacts` to read directly from these pre-calculated columns instead of executing correlated subqueries for every row, and added a generated expression index on `(total_messages_received + total_messages_sent) DESC` to optimize the sorting step. This drastically reduces CPU and I/O overhead for analytics endpoints.
