...
## 2026-03-02 - Optimizing Conversational Report Generation with Active Contact Limiting
**Learning:** `MessageRepository.getConversationsForReport` was retrieving all messages for the current day across all contacts, loading everything into memory to group and sort them, even when only the top active contacts were needed (e.g. for brief reports). This memory slicing approach is slow when there are many long-tail conversations.
**Action:** Implemented a two-step optimized query approach when `limitContacts` is provided. First, fetch only the `contact_id` of the most active contacts via a lightweight `COUNT(*)` aggregation grouping query. Then, fetch the full message payloads only for those `contact_id`s using an `IN (...)` clause. This dramatically reduces memory pressure and DB transmission, yielding a ~3.8x speedup (~750ms -> ~190ms) on a heavy dataset.
