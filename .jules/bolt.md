...
## 2026-03-02 - Eliminate N+1 Correlated Subqueries
**Learning:** `MessageRepository.getTopContacts` originally queried the `contacts` table while executing correlated subqueries against the `messages` table on every row to compute `messages_received` and `messages_sent`. This caused massive N+1 slowdowns as the messages table grew.
**Action:** Replaced the correlated subqueries with direct selections of the pre-calculated `total_messages_received` and `total_messages_sent` columns from the `contacts` table. Benchmarks showed query time dropping from ~2001ms to ~70ms (for 1000 requests). Always leverage pre-aggregated/cached statistical columns if available, rather than re-aggregating on the fly.
