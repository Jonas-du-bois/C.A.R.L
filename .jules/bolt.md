...
## 2026-03-03 - Optimizing Global Aggregations with Pre-calculated Data
**Learning:** Full table `COUNT(*)` scans on growing tables like `messages` become increasingly slow over time.
**Action:** Leveraged pre-calculated `total_messages_received` and `total_messages_sent` fields in the `contacts` table and aggregated them using `SUM()` (wrapped in `CAST(COALESCE(..., 0) AS INTEGER)` for safety). Always look for pre-existing aggregated counters instead of performing full table scans for dashboard stats.
