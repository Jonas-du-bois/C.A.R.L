...
## 2026-03-03 - Avoiding N+1 Correlated Subqueries for Top Contacts
**Learning:** `MessageRepository.getTopContacts` was executing a correlated subquery for every contact to count messages (`SELECT COUNT(*) FROM messages m WHERE m.contact_id = c.id...`), leading to severe N+1 query overhead as the database size grows.
**Action:** Replaced the subqueries with the pre-calculated `total_messages_received` and `total_messages_sent` columns already available in the `contacts` table. Benchmarks showed a ~264x speedup (1.807s -> 6.843ms for 100 queries of 50 contacts). Always use pre-calculated aggregates when available instead of full-table counts in hot paths.
