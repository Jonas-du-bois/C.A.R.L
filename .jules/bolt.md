...
## 2025-05-04 - Optimize getTopContacts to Avoid Correlated Subqueries
**Learning:** `MessageRepository.getTopContacts` was using correlated subqueries (`SELECT COUNT(*) FROM messages ...`) to calculate `messages_received` and `messages_sent` dynamically for each contact, causing N+1 query performance issues (an O(N*M) table scan). Benchmarks showed this took ~32.479s for 100 queries on 1000 contacts.
**Action:** Changed the query to directly select the pre-calculated `total_messages_received` and `total_messages_sent` columns from the `contacts` table. This avoids the correlated subqueries and reduced query time from ~32.479s to ~14ms for 100 queries, representing a massive speedup. Always prefer using pre-calculated aggregate counters maintained via triggers/application logic over dynamic per-row aggregations in hot paths.
