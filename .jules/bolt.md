...
## 2026-03-03 - Removing Redundant Queries
**Learning:** The daily report generation logic was fetching flat messages using `getMessagesForReport` and grouped messages using `getConversationsForReport`. This caused redundant database load. The `messages` parameter was not even used in `#formatBasicReport`.
**Action:** Removed the redundant `getMessagesForReport` method from both `CronService` and `MessageRepository`. Calculate `messagesCount` from the `conversations` array instead.
