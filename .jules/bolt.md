...
## 2026-06-08 - Optimize duplicate message checking with SELECT 1
**Learning:** The application was using `MessageRepository.getMessageById` to check for duplicate messages in the hot path of `Application.js` (for every incoming and outgoing message). This method performs a `SELECT * ` with a `JOIN` on the contacts table, which adds unnecessary overhead when we only need to know if the row exists.
**Action:** Added a lightweight `MessageRepository.messageExists` method that uses `SELECT 1` without a JOIN, and updated `Application.js` to use it. A benchmark showed a 3.13x speedup for this check, reducing database overhead in the hot path.
