...
## 2026-07-06 - Optimize Database Aggregate Queries
**Learning:** Calculating global and per-user message counts by repeatedly counting rows in the `messages` table (via subqueries) was highly inefficient. The `contacts` table maintains pre-calculated aggregates (`total_messages_received`, `total_messages_sent`). Using these pre-calculated columns avoids expensive full-table scans.
**Action:** Updated `getGlobalStats` and `getTopContacts` in `MessageRepository.js` to use `SUM()` and direct selection of the aggregate columns in the `contacts` table. Benchmarks indicate a ~7x speedup for global stats and ~300x speedup for sorting top contacts. Always prefer using maintained pre-calculated aggregates over expensive raw row counting.
