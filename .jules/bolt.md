...
## 2026-03-03 - Optimizing Top N Conversations Query
**Learning:** `MessageRepository.getConversationsForReport(maxMessages, limitContacts)` was fetching, joining, and iterating over ALL messages for ALL contacts for the current day, only to discard most of them in the caller logic when `limitContacts` was specified (e.g., `TelegramCommandHandler` requesting only the top 5). This caused significant latency and memory overhead scaling linearly with the total number of daily messages and active users.
**Action:** Implemented a two-step query optimization. When `limitContacts` is present, it first executes a lightweight `GROUP BY contact_id ORDER BY COUNT(*) DESC LIMIT N` on the `messages` table to identify the top active contact IDs. Then, it modifies the main expensive query (with JOINs) to filter by `AND contact_id IN (...)`. Benchmarks demonstrated a ~71% reduction in execution time for high-volume scenarios. Always fetch only the necessary subset of IDs before executing large JOINs when generating top-N reports.
