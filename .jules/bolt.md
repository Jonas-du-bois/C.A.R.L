...
## 2025-03-02 - O(N) Top Contacts Retrieval
**Learning:** `MessageRepository.getTopContacts` was performing correlated subqueries on the `messages` table to count incoming and outgoing messages per contact `(O(N*M))`. However, the `contacts` table already accurately tracks `total_messages_received` and `total_messages_sent` (used in the `ORDER BY` clause).
**Action:** Replaced the correlated subqueries with direct reads of the pre-calculated columns `total_messages_received as messages_received` and `total_messages_sent as messages_sent`. This avoids querying the massive `messages` table entirely, reducing query time from ~631ms to ~9ms (for 100 queries on a small dataset) and significantly improving performance as message volume grows. Always prefer pre-calculated aggregations over on-the-fly subqueries on large tables.
