...
**Learning:** `MessageRepository.getTopContacts` was performing correlated subqueries (`SELECT COUNT(*)`) on the `messages` table for every contact to calculate received and sent message counts, and explicitly sorting by the sum. This resulted in O(N) subquery execution and table scans.
**Action:** Refactored the query to read from the already maintained pre-calculated columns `total_messages_received` and `total_messages_sent` on the `contacts` table. Added an expression index `idx_contacts_total_msgs ON contacts((total_messages_received + total_messages_sent) DESC)` to back the sorting operation, eliminating explicit file sorts and avoiding the N+1 problem. Always leverage pre-calculated stats columns and expression indexes for frequent aggregation queries.
