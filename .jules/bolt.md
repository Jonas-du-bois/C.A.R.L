...
## 2026-05-09 - Avoiding N+1 Subqueries in Top Contacts
**Learning:** In `MessageRepository.getTopContacts`, fetching message counts using correlated subqueries (`(SELECT COUNT(*) FROM messages ...)`) for each returned contact resulted in poor performance, especially as the messages table grows. The benchmark showed 100 queries taking 10.710s.
**Action:** Replaced the correlated subqueries with direct reads of the pre-calculated `total_messages_received` and `total_messages_sent` columns already present on the `contacts` table. This improved performance by ~700x (14.677ms vs 10.710s). Always prefer reading pre-calculated aggregates over on-the-fly correlated subqueries.
