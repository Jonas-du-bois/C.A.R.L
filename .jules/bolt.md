...
## 2026-03-03 - Favor Pre-calculated Aggregates over Table Scans
**Learning:** `MessageRepository.getGlobalStats` previously performed full table scans (`COUNT(*)`) on the `messages` table for each direction. As the table grows, this query becomes significantly slower. However, the `contacts` table already maintains exact, pre-calculated totals (`total_messages_received`, `total_messages_sent`) for every user. Summing these pre-calculated integers from the smaller `contacts` table is inherently much faster than counting rows in the primary, high-volume `messages` table.
**Action:** Replaced `COUNT(*)` queries on `messages` with `SUM()` queries on the respective total columns in the `contacts` table. Always verify if the necessary aggregated data is already maintained atomically elsewhere in the schema before defaulting to expensive `COUNT(*)` operations on large tables.
