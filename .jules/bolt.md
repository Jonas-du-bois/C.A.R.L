...
## 2026-03-02 - Eliminate Correlated Subqueries Using Pre-Calculated Aggregates
**Learning:** `MessageRepository.getTopContacts` was performing correlated subqueries (`SELECT COUNT(*) FROM messages...`) to determine `messages_received` and `messages_sent` for each contact. This led to N+1 queries being executed internally by SQLite, causing a severe performance degradation as the `contacts` table grew.
**Action:** Since `total_messages_received` and `total_messages_sent` are already tracked and incremented atomically on the `contacts` table, replaced the subqueries with direct column lookups (`c.total_messages_received`, `c.total_messages_sent`). Benchmark showed an ~84% reduction in query time (from ~1274ms to ~197ms for 1000 calls). Always leverage pre-calculated aggregate fields on parent tables to avoid re-evaluating counts on large child tables.
