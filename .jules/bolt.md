...
## $(date +%Y-%m-%d) - Avoiding JOINs for Simple Existence Checks
**Learning:** `Application.js` checked for duplicate messages by calling `MessageRepository.getMessageById`, which executed a query fetching all message columns and performing a `JOIN` with the `contacts` table. This added unnecessary overhead because the application only needed to know *if* the record existed, not its contents.
**Action:** Implemented `MessageRepository.messageExists` using a lightweight `SELECT 1 FROM messages WHERE message_id = ? LIMIT 1` query. Replaced `getMessageById` with `messageExists` in duplicate detection logic. Always use `SELECT 1` with a `LIMIT` when checking for record existence instead of fetching full entities, especially on high-frequency code paths.
