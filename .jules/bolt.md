...
## 2026-06-20 - Replace expensive full row selects with lightweight existence checks
**Learning:** Checking for duplicate messages using `getMessageById` executes an unnecessary `JOIN` with the `contacts` table and fetches all columns, which is computationally expensive for a simple true/false check.
**Action:** Always use a lightweight `SELECT 1` query (`messageExists`) when you only need to determine if a record exists in the database.
