...
## 2026-03-03 - N+1 Correlated Subqueries in getTopContacts
**Learning:** `MessageRepository.getTopContacts` was executing a correlated subquery for each returned contact to count their incoming and outgoing messages from the `messages` table. This causes a massive performance degradation (N+1 query problem) because SQLite executes these subqueries for every row returned. The `contacts` table already maintained pre-calculated `total_messages_received` and `total_messages_sent` counters via atomic UPSERTs, making these subqueries entirely redundant.
**Action:** Replaced the correlated subqueries with direct column selections `c.total_messages_received as messages_received` and `c.total_messages_sent as messages_sent`. Benchmarks showed an improvement of ~10,000x (72.7s down to 7ms for 100 queries). Always leverage existing pre-calculated aggregations instead of re-calculating them on the fly for read-heavy operations.
