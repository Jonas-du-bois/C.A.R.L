...
**Vulnerability:** Fetch errors in `TelegramService` (e.g. DNS failures) leaked the full URL including the Telegram bot token in `error.message` and `error.cause`, which were then written to logs via `console.error`.
...
**Prevention:** Implemented `#sanitizeError` to deeply sanitize Error objects and strings, replacing the token with `[HIDDEN_TOKEN]` before any logging occurs.
