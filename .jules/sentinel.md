...
**Vulnerability:** The Telegram bot token, embedded in the URL, could be leaked in `console.error` logs if `fetch` fails or returns a non-OK response. Standard HTTP clients leak the full URL on failure.
...
**Prevention:** Implemented `#sanitizeError` method to intercept and replace the bot token with `[HIDDEN_TOKEN]` in error objects, strings, stack traces, and nested error causes before logging.
