...
## 2025-05-29 - Secret Leakage via Error Logging
**Vulnerability:** CRITICAL risk in `TelegramService`. The service was directly logging raw error objects and API response texts from `fetch` calls. Because the Telegram API includes the `botToken` in the URL, a failed request would log the full URL, exposing the secret token in plain text in the application logs.
**Learning:** Standard HTTP clients like `fetch` will often include the full requested URL in their error messages. Any service interacting with APIs that embed secrets in the URL must actively intercept and scrub error objects and strings before logging them.
**Prevention:** Implemented `#sanitizeError` in `TelegramService` to explicitly regex-escape the `botToken` and replace it with `[HIDDEN_TOKEN]` in Error objects (including `message`, `stack`, and `cause`) and strings before they are passed to `console.error`.
