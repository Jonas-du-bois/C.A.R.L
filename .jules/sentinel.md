...
## 2025-05-29 - Fetch URL Secret Leakage
**Vulnerability:** CRITICAL security risk in `TelegramService`. Error objects and strings generated by standard HTTP clients like `fetch` can leak the full request URL upon failure, exposing hardcoded secrets (e.g. `botToken`) embedded within the URL in API responses or generic network errors.
**Learning:** Any service interacting with APIs that embed secrets directly into the URL must actively intercept and thoroughly scrub all error objects (including deep properties like `cause`, `stack`, `message`, and custom fields) and simple strings before allowing them to be logged or returned.
**Prevention:** Implemented a `#sanitizeError` method in `TelegramService` that recursively searches for the `botToken` via a regex-escaped string matcher and replaces all occurrences with `[HIDDEN_TOKEN]` to ensure secrets are sanitized before being logged.
