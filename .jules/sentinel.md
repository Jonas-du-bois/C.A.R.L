...
## 2025-05-29 - Secret Leakage via Error Objects
**Vulnerability:** CRITICAL secret leakage in `TelegramService`. Error objects and string messages from failed `fetch` calls implicitly contained the bot token because the Telegram API endpoint includes the token in the URL.
...
**Prevention:** Implemented `#sanitizeError` in `TelegramService` to recursively regex-escape and replace the bot token in Error messages, stacks, causes, and raw strings before logging.
