...
## 2025-05-29 - Telegram API Token Leakage in Logging
**Vulnerability:** API requests using standard HTTP clients like `fetch` can implicitly embed full URLs in the returned Error objects (e.g., `fetch failed: https://.../bot<SECRET_TOKEN>/sendMessage`), which are then blindly written to logs via `console.error`. This could expose the bot token to anyone with log access.
**Learning:** Any service interacting with APIs that embed secrets in the URL must actively intercept and sanitize errors and messages before logging them.
**Prevention:** Implemented a recursive `#sanitizeError` method in `TelegramService` that actively finds and replaces all occurrences of the bot token with `[HIDDEN_TOKEN]` in `Error` objects (including messages, stacks, and causes) and string primitives.
