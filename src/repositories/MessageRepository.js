...
export class MessageRepository {
  ...
  getTopContacts(limit = 10) {
    // ⚡ Bolt: Optimized query to read pre-calculated aggregates directly from the contacts table
    // instead of running correlated SELECT COUNT(*) subqueries on the messages table (N+1 overhead).
    return this.#db.prepare(`
      SELECT c.*, 
