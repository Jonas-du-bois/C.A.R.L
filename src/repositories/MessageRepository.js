...
export class MessageRepository {
  ...
  getTopContacts(limit = 10) {
    // ⚡ Bolt: Removed expensive correlated subqueries for messages_received/messages_sent.
    // We now read directly from the pre-calculated total_messages_received and total_messages_sent
    // columns in the contacts table, which are atomically updated during message ingestion.
    // This reduces query complexity from O(N*M) to O(N log N) (due to ORDER BY).
    return this.#db.prepare(`
      SELECT c.*, 
