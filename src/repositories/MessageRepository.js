...
export class MessageRepository {
  ...
  getTopContacts(limit = 10) {
    // ⚡ Bolt: Optimized query by replacing correlated subqueries (N+1 like) on the `messages` table
    // with direct reads from the pre-calculated `total_messages_received` and `total_messages_sent` columns in the `contacts` table.
    return this.#db.prepare(`
      SELECT c.*, 
