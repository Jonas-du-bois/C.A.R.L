...
export class TelegramService {
  ...
  /**
   * Sanitizes an error object or string by replacing the bot token with [HIDDEN_TOKEN]
   * @param {Error|string} error The error to sanitize
   * @returns {Error|string} A new sanitized error object or string
   */
  #sanitizeError(error) {
    if (!error) return error;
    if (!this.#botToken) return error;

    // Escape special regex characters in the token just in case
    ...
    if (error instanceof Error) {
      const sanitizedMessage = error.message.replace(tokenRegex, '[HIDDEN_TOKEN]');
      const newError = new Error(sanitizedMessage);

      // Preserve the original error type (e.g., TypeError, ReferenceError)
      newError.name = error.name;

      // Copy other properties, replacing token if they are strings
