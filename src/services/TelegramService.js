...
export class TelegramService {
  ...
  /**
   * Sanitizes errors to prevent secret leakage in logs.
   * Replaces the bot token with [HIDDEN_TOKEN] in Error objects and strings.
   */
  #sanitizeError(error) {
    if (!error || !this.#botToken) return error;

    // Create a safe regex string from the bot token to escape special characters
    ...
    if (error instanceof Error) {
      // Create a new Error instance to preserve the original type without mutating
      const newError = new Error(error.message ? error.message.replace(tokenRegex, hiddenToken) : '');
      newError.name = error.name;

      // Copy other properties
      for (const key of Object.keys(error)) {
        if (key !== 'message' && key !== 'name' && key !== 'stack' && key !== 'cause') {
          newError[key] = typeof error[key] === 'string' ? error[key].replace(tokenRegex, hiddenToken) : error[key];
        }
      }
