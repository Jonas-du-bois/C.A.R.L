...
export class TelegramService {
  ...
  /**
   * Sanitizes error objects and strings to prevent bot token leakage
   */
  #sanitizeError(error) {
    ...
    if (error instanceof Error) {
      const sanitizedMsg = error.message.replace(tokenRegex, hiddenToken);
      ...
      for (const [key, value] of Object.entries(error)) {
        if (typeof value === 'string') {
          newError[key] = value.replace(tokenRegex, hiddenToken);
        } else {
          newError[key] = value;
        }
      }
  ...
  async #pollUpdates() {
    ...
    try {
      ...
      // Silently ignore polling errors in production, but sanitize in dev
